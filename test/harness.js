...
const mkEl=()=>({style:{},classList:{toggle(){}},addEventListener(){},set textContent(v){css=v},set innerHTML(v){this._h=v},get innerHTML(){return this._h}});
global.document={readyState:'complete',body:{appendChild(e){els.push(e)}},head:{appendChild(){}},createElement:mkEl,querySelectorAll:()=>[],addEventListener(){}};
...
setTimeout(()=>{
  ...
  setTimeout(()=>{ fs.writeFileSync('test/panel.html',`<style>body{background:#222;margin:0;padding:20px}${css}</style><div id="fh" style="position:static">${els[0].innerHTML}</div>`); console.log('ok'); process.exit(0)},800);
},3000);
